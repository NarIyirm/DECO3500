    ...
    <UCard class="w-full max-w-3xl mx-auto bg-white bg-opacity-90 rounded-lg shadow-lg p-6 md:p-8 transform hover:-translate-y-2 transition duration-300">
      ...
      <div class="mb-6">
        ...
        <ul class="space-y-3 mt-4 flex flex-wrap justify-center">
          <li v-for="(participant, index) in event2Participants" :key="index" class="flex items-center m-2">
      ...
      <!-- 最近参与者 -->
      <div v-if="filteredEventParticipants.length > 0" class="mb-6">
        <ul class="space-y-3 mt-4 flex flex-wrap justify-center">
          <li v-for="(participant, index) in filteredEventParticipants" :key="index" class="flex items-center m-2">
            <img :src="participant.avatar" alt="Participant Avatar" class="w-10 h-10 md:w-12 md:h-12 rounded-full shadow-md mr-3 transform hover:scale-105 transition duration-300 cursor-pointer object-cover" />
            <div class="flex flex-col text-center">
              <span class="font-medium text-gray-800">{{ participant.name }}</span>
              <span class="text-sm text-gray-500">{{ participant.time }}</span>
...
const filteredEventParticipants = computed(() => {
  return participants.value.filter(participant => participant.time !== 'You have not participated yet.');
});

// 控制二维码弹窗显示
const showQrCode = ref(false);
const qrCodeValue = 'http://20.11.49.12:10805/event3'; // 扫描二维码后显示的内容
...
const signUp = async () => {
  ...
  event2Participants.value = event2Participants.value.map(participant => ({
    ...participant,
    time: `Signed up at ${currentTime}`
  }));

  // 发送更新后的参与者信息到后端
  await fetch('http://20.11.49.12:10804/update-participants/event2', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
    body: JSON.stringify({ updatedParticipants: event2Participants.value })
  });
};
