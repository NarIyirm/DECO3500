    ...
    <UCard class="w-full max-w-3xl mx-auto bg-white bg-opacity-90 rounded-lg shadow-lg p-6 md:p-8 transform hover:-translate-y-2 transition duration-300">
      ...
      <!--<div v-if="filteredEvent2Participants.length > 0" class="mb-6">-->
        ...
        <ul class="space-y-3 mt-4 flex flex-wrap justify-center">
          <li v-for="(participant, index) in event3Participants" :key="index" class="flex items-center m-2">
...
const qrCodeValue = 'http://20.11.49.12:10805/event'; // 扫描二维码后显示的内容
...
const signUp = async () => {
  ...
  event3Participants.value = event3Participants.value.map(participant => ({
    ...participant,
    time: `Signed up at ${currentTime}`
  }));

  // 发送更新后的参与者信息到后端
  await fetch('http://20.11.49.12:10804/update-participants/event3', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
    body: JSON.stringify({ updatedParticipants: event3Participants.value })
  });
};
